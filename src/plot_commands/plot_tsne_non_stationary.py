...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate t-SNE visualizations of selected points per strategy for non-stationary experiments"
    )
    parser.add_argument(
        "--penalty",
        type=str,
        required=True,
        help="Which cost penalty to use (e.g., '0.25')",
    )
    parser.add_argument(
        "--benchmark",
        type=str,
    ...
    base_results_dir = (
        f"results/{args.penalty}/non_stationary/{args.embedding_model}/{args.benchmark}"
    )
