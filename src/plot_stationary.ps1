<#
.SYNOPSIS
    Generates the plots for the stationary experiments (AUC vs Cost, and optionally t-SNE).

.EXAMPLE
    .\plot_stationary.ps1 -Penalty "0.25" -Benchmark "RouterBench"

.EXAMPLE
    .\plot_stationary.ps1 -Penalty "0.25" -Benchmark "RouterBench" -Seeds 1,2,3 -Tsne -StrategyPrefixes "rand_","act_unc","oracle"
#>
...
Write-Host "Generating Stationary Experiment Plots" -ForegroundColor Cyan
Write-Host "======================================" -ForegroundColor Cyan
...
Write-Host "Step 1: Generating AUC vs Cost plots..." -ForegroundColor Green
$aucCommand = "python -m plot_commands.plot_all_stationary --penalty $Penalty --benchmark $($Benchmark.ToLower())"
...
if ($Tsne) {
    ...
    $tsneCommand = "python -m plot_commands.plot_tsne_stationary --penalty $Penalty --benchmark $Benchmark --seeds $($Seeds -join ' ') "
...
Write-Host "  - AUC plots: plots_benchmarks/auc_vs_cost/$($Benchmark.ToLower())" -ForegroundColor Gray
if ($Tsne) {
    Write-Host "  - t-SNE plots: plots_benchmarks/tsne_selections/$Benchmark" -ForegroundColor Gray
}
